...
targetPath = "./results"					
numberOfThreads =math.ceil(multiprocessing.cpu_count()/2) # defines how many detection processes will be run in parallel	#
verbose = False
#########################################################################################
	...
	for i in tmpPATH:
		...
		if os.path.isdir(buildPATH) != 1:
			try:
				...
				if verbose: print ("Creation of the directory failed: "+buildPATH)
				exit()
			else:
				if verbose: print ("Successfully created the directory:" +buildPATH)
		else:
			if verbose: print("SUCCESSFULL: target dir: '"+buildPATH+"' already exists!")
	time.sleep(1)
...
def run_docking(i, ToDo_numberOfCalculation):
	...
	if (os.path.isfile(checkfile) != True):
		...
		cmd = subprocess.Popen(["vina", "--config", "./conf.txt", "--ligand", PDBQTpath+PDBIDs[i],
		                        "--out", "./results/"+basename+"/"+basename+"_res.pdbqt"], stdout=subprocess.DEVNULL, stderr=subprocess.STDOUT)
	...
	printProgressBar(currentNumberOfCalcs[0], ToDo_numberOfCalculation,
...
printProgressBar(0, ToDo_numberOfCalculation, prefix = 'Progress:', suffix = 'Complete', length = 50)
if (ToDo_numberOfCalculation != 0):
	a = Parallel(n_jobs=int(numberOfThreads), prefer="threads")(delayed(run_docking)(
	    i, ToDo_numberOfCalculation) for i in range(0, ToDo_numberOfCalculation))
	printProgressBar(currentNumberOfCalcs[0], ToDo_numberOfCalculation, prefix = 'Progress:', suffix = 'Complete', length = 50)
print("final write...")
